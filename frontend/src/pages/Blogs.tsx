            ...
            <div className="w-full max-w-4xl flex flex-col">
                {blogs.length === 0 ? (
                    <div className="p-8 text-center text-term-fg border border-term-fg/30 border-dashed opacity-70 my-8">
                        ...
                        <p className="text-sm border-t border-term-fg/30 pt-2 inline-block">No system logs discovered in this partition. <br /> Run ./publish_new.sh to write.</p>
                    </div>
                ) : (
                    blogs.map((blog: { id: string; author: { name: string }; title: string; content: string; createdAt: string }) => <BlogCard
                        key={blog.id}
                        id={blog.id}
